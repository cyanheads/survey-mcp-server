...
This example from the `survey_submit_response` tool demonstrates how to build a rich, LLM-facing output that provides comprehensive context, including visual progress, scoring feedback, newly available questions, and clear next steps. This empowers the LLM to continue the conversation intelligently without needing to make extra tool calls.
...
function responseFormatter(result: SubmitResponseResponse): ContentBlock[] {
  ...
  // 2. Score Display (if scoring is enabled)
  const scoreSection =
  ...
  // 3. Highlight Newly Unlocked Questions
  const newlyUnlocked =
  ...
  // 5. Give High-Level Conversational Guidance
  const guidance =
...
- **Give the LLM Everything It Needs:** The output includes progress, scores (when available), newly unlocked questions (with text and reasons), and suggestions for what to ask next. This prevents the LLM from having to make extra tool calls (`get_progress`, `get_question`) to figure out what to do.
...
- **Conditional Display:** Score information only appears when scoring is enabled (optional fields), ensuring the output remains clean for regular surveys.
- **Structure for Clarity:** The output is organized with clear sections (Progress → Score → New Questions → Suggestions → Guidance) to create a logical flow.
- **Provide Actionable Guidance:** The final text gives the LLM a direct suggestion (e.g., "Ready to complete the survey"), guiding it toward the correct next action.
...
- **`survey/`**: Service orchestrator pattern with `ISurveyProvider` interface, domain-specific storage, and specialized operations (conditional logic, progress tracking, scoring, analytics with pagination)
